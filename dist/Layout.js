...
export default function Layout({ icon = init.icon, color, gradient, size = init.size, }) {
    ...
    return (_jsxs("svg", { width: `${size}px`, height: `${size}px`, viewBox: "0 0 24 24", fill: `${gradient === undefined ? color : `url(#${elementId}`}`, fillRule: "evenodd", children: [gradient !== undefined ? (_jsx("defs", { children: _jsx("linearGradient", { id: elementId, x1: _gradient.from.x, y1: _gradient.from.y, x2: _gradient.to.x, y2: _gradient.to.y, children: _gradient.steps.map((item, i) => {
                        return (_jsx("stop", { offset: `${item.offset}%`, stopColor: item.color, stopOpacity: `${item.opacity}%` }, i));
                    }) }) })) : null, _jsx("path", { d: icon })] }));
}
